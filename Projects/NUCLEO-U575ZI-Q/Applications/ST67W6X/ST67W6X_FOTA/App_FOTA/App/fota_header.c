...
int32_t fota_header_ParseJsonToStruct(const uint8_t *buffer, size_t buffer_size, FotaHeader_t *header)
{
  ...
  name = cJSON_GetObjectItem(p_fw_header_json, MAGIC_NUM);
  if (cJSON_IsString(name) && (name->valuestring != NULL))
  {
    /* Verify the magic number is the one expected */
    if (memcmp(name->valuestring, MAGIC_NUMBER, strlen(MAGIC_NUMBER)) != 0)
    {
      ret = FOTA_HEADER_ERR_INVALID_MAGIC;
      goto _err1;
    }
    memcpy(header->magic_num, name->valuestring, strlen(MAGIC_NUMBER));
  }
  else
  {
    goto _err1;
  ...
  protocol_version = cJSON_GetObjectItem(p_fw_header_json, PROTOCOL_VERSION);
  if (cJSON_IsString(protocol_version) && (protocol_version->valuestring != NULL))
  {
    /* Verify the protocol version is the one expected */
    if (memcmp(protocol_version->valuestring, FOTA_PROTOCOL_VERSION, strlen(FOTA_PROTOCOL_VERSION)) != 0)
    {
      ret = FOTA_HEADER_ERR_INVALID_HEADER_VER;
      goto _err1;
    }
    memcpy(header->protocol_version, protocol_version->valuestring, strlen(FOTA_PROTOCOL_VERSION));
  }
  else
  {
    goto _err1;
  ...
  info_fields = cJSON_GetObjectItem(p_fw_header_json, "info");
  if (!cJSON_IsNull(info_fields))
  {
    /* Looking for the NCP firmware type info in the JSON */
    const cJSON *info_fw_type = cJSON_GetObjectItem(info_fields, INFO_FIRMWARE_TYPE);
    ...
    const cJSON *info_target = cJSON_GetObjectItem(info_fields, INFO_DATA_TYPE);
    ...
    const cJSON *info_board_name = cJSON_GetObjectItem(info_fields, INFO_PREFIX_BOARD_NAME);
    ...
    const cJSON *info_board_rev = cJSON_GetObjectItem(info_fields, INFO_BOARD_REVISION);
  ...
  file_hash = cJSON_GetObjectItem(p_fw_header_json, FILE_HASH);
  if (!cJSON_IsArray(file_hash))
  {
    LogError("file_hash is not a valid JSON array\n");
    goto _err1;
  }
  /* Check the expected size of the hash array and the one from the JSON file */
  array_size = cJSON_GetArraySize(file_hash);
  ...
  {
    cJSON *item = cJSON_GetArrayItem(file_hash, i);
  ...
  host_ver = cJSON_GetObjectItem(p_fw_header_json, STM32_APP_VER);
  if (cJSON_IsString(host_ver) && (host_ver->valuestring != NULL))
  {
    memcpy(header->stm32_app_ver, host_ver->valuestring, sizeof(header->stm32_app_ver));
  }
  else
  {
    goto _err1;
  ...
  st67_ver = cJSON_GetObjectItem(p_fw_header_json, ST67_VER);
  if (cJSON_IsString(st67_ver) && (st67_ver->valuestring != NULL))
  {
    memcpy(header->st67_ver, st67_ver->valuestring, sizeof(header->st67_ver));
  }
  else
  {
    goto _err1;
...
int32_t fota_header_CmpVer(const uint8_t *current_ver, const uint8_t *new_ver)
{
  ...
  if (sscanf((const char *)current_ver, "%" SCNi32 ".%" SCNi32 ".%" SCNi32, &major, &minor, &patch) != 3)
  {
    LogError("Invalid version format\n");
    return FOTA_HEADER_ERR;
  }
  /* Convert the version string to an integer for comparison */
  current_v = ((major & 0xFF) << 16) | ((minor & 0xFF) << 8) | (patch & 0xFF);

  /* Scan for the x.y.z format version and saved them into int32_t variables */
  if (sscanf((const char *)new_ver, "%" SCNi32 ".%" SCNi32 ".%" SCNi32, &major, &minor, &patch) != 3)
  {
    LogError("Invalid version format\n");
    return FOTA_HEADER_ERR;
  }
