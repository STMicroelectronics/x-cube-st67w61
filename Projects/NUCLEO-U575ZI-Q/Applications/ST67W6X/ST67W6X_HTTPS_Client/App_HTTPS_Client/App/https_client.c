...
int32_t https_client_weather(char *city, float latitude, float longitude)
{
  ...
  char certificate[16] = "weather.crt";
  char url[256] = {0};
  char *token = NULL;
  char *address = NULL;
  ...
  if (W6X_Net_Inet_pton(AF_INET, address, (const void *) &addr.u_addr.ip4) != 0)
  {
    if (W6X_Net_GetHostAddress(server_name, https_server_addr) != W6X_STATUS_OK)
    {
      LogError("IP Address identification failed\n");
      return -1;
    }
    addr.u_addr.ip4 = ATON_R(https_server_addr);
  }

  /* Set the HTTPS settings */
  settings.https_certificate = certificate;
  settings.https_certificate_len = strlen(certificate) + 1;
