...
void main_app(void)
{
  ...
  ret = W6X_Net_SNTP_GetConfiguration(&SNTP_Enable, &SNTP_Timezone, NULL, NULL, NULL);
  ...
  if (W6X_STATUS_OK == W6X_Net_SNTP_GetTime(&net_time))
  {
    LogInfo("SNTP Time: %s\n", net_time.raw);
  ...
  if (W6X_STATUS_OK == W6X_MQTT_Connect(&mqtt_config))
  {
