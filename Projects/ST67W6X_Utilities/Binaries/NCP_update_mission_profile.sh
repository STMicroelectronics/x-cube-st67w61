...
QConn_Flash/QConn_Flash_Cmd-ubuntu --port $COMADDR --config "$current_dir/NCP_Binaries/mission_flash_prog_cfg.ini" --efuse="$current_dir/NCP_Binaries/efusedata.bin"
if [ $? -ne 0 ]; then
  echo -e "${RED}!!!! Error detected !!!!${NC}"
  exit 1
fi

if [ "$BOARD_ID" = "NUCLEO-H7S3L8" ]; then
...
elif [ "$BOARD_ID" = "NUCLEO-N657X0-Q" ]; then
    echo "Set the boot mode in development mode (BOOT1 switch position is 2-3, BOOT0 switch position doesn't matter)"
    echo "Press any key to continue..."
    read -r junk
